defmodule TestProjectWeb.OrderController do
  ...
  def new(conn, _params) do
    changeset = Shop.change_order(%Order{items: []})
  ...
  def edit(conn, %{"id" => id}) do
    order = Shop.get_order!(id)
    changeset = Shop.change_order(order)
  ...
  def update(conn, %{"id" => id, "order" => order_params}) do
    ...
    case Shop.update_order(order, order_params) do
      {:ok, order} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    order = Shop.get_order!(id)
    {:ok, _order} = Shop.delete_order(order)
